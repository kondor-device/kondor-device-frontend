...
export default function ImagePicker({
  ...
  return (
    <div className="flex flex-col deskxl:flex-row deskxl:justify-between gap-y-3 deskxl:gap-x-5 laptop:max-w-[340px] deskxl:max-w-full h-full deskxl:max-h-[466px]">
      <div
        className="flex justify-between items-center max-w-[306px] laptop:max-w-[340px] laptop:size-[340px] deskxl:max-w-[466px] deskxl:size-[466px] bg-white 
  aspect-[1/1] rounded-[11px] laptop:rounded-[40px] overflow-hidden"
      >
        <Image
      ...
      >
        <ul className="flex flex-row deskxl:flex-col gap-y-4 gap-x-4 deskxl:gap-y-[35px] w-fit h-fit mx-auto my-auto">
          {photos.map(({ url, alt }, idx) => (
            <li
              key={idx}
              className={`cursor-pointer flex items-center justify-center size-[46px] laptop:size-[62px] deskxl:size-[60px] bg-white rounded-[2px] 
